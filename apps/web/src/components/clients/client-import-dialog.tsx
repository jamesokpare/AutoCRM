...
const TEMPLATE =
  "name,phone,email,whatsapp,address,preferredChannel\n" +
  "Jane Doe,+2348012345678,jane@example.com,+2348012345678,12 Allen Ave Lagos,WHATSAPP\n" +
...
export function ClientImportDialog({ trigger }: { trigger: React.ReactElement }) {
  ...
  function onImport() {
    ...
    startTransition(async () => {
      ...
      const { created, skipped: serverSkipped } = res.data;
      toast.success(
        `Imported ${created} client${created === 1 ? "" : "s"}` +
          (serverSkipped ? ` · ${serverSkipped} skipped` : ""),
      );
      setOpen(false);
      reset();
      router.refresh();
